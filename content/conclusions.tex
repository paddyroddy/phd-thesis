...
The matrix is filled in parallel in \texttt{Python} using \texttt{multiprocessing}, and the spherical harmonic transforms are computed in \texttt{C} using \texttt{SSHT}\cref{fn:ssht}.
This may be sped up further by utilising the new \texttt{ducc0} backend for \texttt{SSHT}\cref{fn:ssht}, which may allow for a multithreaded solution.

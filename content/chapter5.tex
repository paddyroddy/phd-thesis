...
\begin{equation}
	\mesh{f}
	%
	= \displaystyle\sum\limits_{i=1}^{\imax} f_{i} \mesh{\psi_{i}},
\end{equation}
%
...
\begin{equation}\label{eq:chapter5_graph_div}
	{(\div F)}_{i}
	%
	= \frac{1}{a_{i}} \displaystyle\sum\limits_{j : (i,j) \in \edges} w_{i j} F_{i j}.
\end{equation}
%
...
\begin{equation}\label{eq:chapter5_graph_laplace_indices}
	{(\laplace{G} f)}_{i}
	%
	= \frac{1}{a_{i}} \displaystyle\sum\limits_{(i,j) \in \edges} w_{i j} (f_{i} - f_{j}).
\end{equation}
%
...
\begin{equation}
	\vb*{K}
	%
	= \diag{\displaystyle\sum\limits_{j: j \neq i} w_{i j}}
\end{equation}
%
...
\begin{equation}\label{eq:chapter5_vertex_weight}
	a_{i}
	%
	= \frac{1}{3} \displaystyle\sum\limits_{j k : (i,j,k) \in \faces} a_{i j k},
\end{equation}
%
...
\begin{equation}
	\mesh{f}
	%
	= \displaystyle\sum\limits_{p=1}^{\imax} \slepian{f} \mesh{\slepian{S}},
\end{equation}
%
...
\begin{equation}
	\mesh{f}
	%
	\approx \displaystyle\sum\limits_{p=1}^{N} \slepian{f} \mesh{\slepian{S}}
	%
...
\cref{sec:chapter4_generating_functions} defines the smooth generating functions that tile the Slepian line to construct the wavelets (\cf{}~\cite{Wiaux2008}).
...
The wavelet generating functions, discussed in \cref{sec:chapter5_generating_functions}, are constructed through the \texttt{S2LET}\footnote{\url{http://astro-informatics.github.io/s2let/}}~\cite{Leistedt2013} code.
...
This chapter generalises \emph{Slepian scale-discretised wavelets on the sphere}~\cite{Roddy2021a} (\cf{} \cref{sec:chapter4}) to Riemannian manifolds, whereby the wavelets themselves are restricted to a region of the manifold.

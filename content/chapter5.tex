...
This work extends Slepian scale-discretised wavelets on the sphere to manifolds, a brief review of which is given in \cref{sec:chapter5_problem_formulation}.
...
The \emph{non-normalised graph Laplacian} refers to setting \(\vb*{A} = \vb*{I}\) in \cref{eq:chapter5_graph_laplace_matrix}; other choices exist, such as the random walk Laplacian which occurs when \(\vb*{A} = \vb*{K}\)~\autocite{VonLuxburg2007}.
...
\input{figures/chapter5/mesh_laplace}

\input{figures/chapter5/eigenhomers}
...
This work considers optimally concentrated functions within a region \(R\) of a manifold \(\manifold{}\), an example of which is shown in \cref{fig:chapter5_region}.

\input{figures/chapter5/example_region_manifold}
...
\end{equation}
%
where \(\vb*{S}\) are the harmonic coefficients of \(\mesh{\slepian{S}}\) (see \cref{eq:chapter4_integrate_region_proof} for proof).
...
Initially, \cref{sec:chapter5_slepian_sifting_convolution} extends the sifting convolution~\autocite{Roddy2021} to the Slepian domain.
...
The sifting convolution~\autocite{Roddy2021} on the manifold, defined in \cref{sec:chapter5_slepian_concentration_problem_manifolds}, can be extended to work with the Slepian functions as a basis.
...
\end{equation}
%
where \(y\) is a point on the manifold and \(\mesh{\slepian{S}}\) are the Slepian functions defined in \cref{sec:chapter5_slepian_concentration_problem_manifolds}.
...
A tiling of the Slepian line defines the wavelets through smooth generating functions --- introduced in \cref{sec:chapter4_generating_functions}.
...
\input{figures/chapter5/homer_tiling}
...
All computations are performed with the \texttt{S2LET}\footnote{\url{http://astro-informatics.github.io/s2let/}}~\autocite{Leistedt2013} code, which enables the construction of the wavelet generating functions discussed in \cref{sec:chapter4_generating_functions}.
...
The Slepian functions of this region are computed by solving the eigenproblem \cref{eq:chapter5_eigenproblem}, and then performing an inverse harmonic transform.
...
\input{figures/chapter5/homer_region}

\input{figures/chapter5/homer_slepian_functions}

\input{figures/chapter5/homer_slepian_eigenvalues}
...
\input{figures/chapter5/homer_wavelets}

\input{figures/chapter5/homer_field}

\input{figures/chapter5/homer_wavelet_coefficients}
...
\input{figures/chapter5/homer_denoising}

\input{figures/chapter5/other_mesh_regions}

\input{tables/chapter5/mesh_denoising}

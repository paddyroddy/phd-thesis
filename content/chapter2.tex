...
\begin{equation}\label{eq:chapter2_wigner_ylm}
	...
	%
	= \sum\limits_{m'=-\ell}^{\ell} \wigner{\ell}{m'}{m}(\rho) \pixel{Y_{\ell m'}},
\end{equation}
%
where \(\wigner{\ell}{m'}{m}(\rho)\) are the \emph{Wigner D matrices}~\cite{Brink1993,Ritchie1999}.
The Wigner D matrices form the \(2\ell+1\)-dimensional representation of the rotation group \(\rotationGroup{}\) and are given by
%
\begin{equation}
	\wigner{\ell}{m'}{m}(\rho)
	%
...
\end{equation}
%
where \(\reducedWigner{\ell}{m'}{m}(\beta)\) are the \emph{reduced d matrices} which are real in this phase convention.
...
\end{equation}
%
Note the order of the indices on \(\wigner{\ell}{m}{m'}(\rho)\) here compared to \cref{eq:chapter2_wigner_ylm}.

...
Presented here is a brief review of (Euclidean) wavelet analysis, for more detail see, \eg{}~\cite{Rioul1991,Graps1995,Addison2005,Valens1999,Kaiser2011}.
...
A scaling function \(\Phi_{k}(t)\), in blue, is typically introduced to capture the lower scales of the underlying function~\cite{Mallat1989}.
...
\begin{equation}
	\text{DWT}_{f}(j,k)
	%
	= \sum\limits_{i} f[t_{i}] \Psi^{j\ast}_{k}[t_{i}],
\end{equation}
%
...
\begin{equation}
	f(t)
	%
	= \sum\limits_{i} \sum\limits_{k} W^{\Phi}_{k} \Phi_{k}[t_{i}]
	%
	+ \sum\limits_{j} W^{\Psi^{j}}_{k} \Psi^{j}_{k}[t_{i}],
\end{equation}
%
where the scaling coefficients (and similarly for the wavelet coefficients \(W^{\Psi^{j}}_{k}\)) are found by
%
\begin{equation}
	W^{\Phi}_{k}
	%
	= \sum\limits_{i} f[t_{i}] \conj{\Phi}_{k}[t_{i}].
\end{equation}
%
...
Continuous~\cite{Torresani1995,Holschneider1996,Freeden1997,Antoine1998,Antoine1999,Antoine2002,Demanet2003,Wiaux2005,Sanz2006,McEwen2006}.
Discrete~\cite{Sweldens1996,Schroder2000,Wiaux2005,Starck2006,Wiaux2008,Starck2009,Leistedt2013,McEwen2019,McEwen2018}.

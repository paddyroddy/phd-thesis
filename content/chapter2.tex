...
The probability of realising some field configuration is a \emph{functional} \(\text{Pr}[f(\vb*{x})]\).
\emph{Correlators} of fields are expectation values of products of fields at different spatial points.
Recall the definition of the two-point correlator (power spectrum)
%
\begin{equation}
	\expval*{f(\vb*{k}) \conj{f}(\vb*{k'})} = \frac{2\pi^{2}}{k^{3}} \mathcal{P}_{f}(k) \delta^{3}(\vb*{k}-\vb*{k'})
\end{equation}
%
...
\begin{subequations}
	\begin{align}
		f(\vb*{k}) & = \int \frac{\dd[3]{\vb*{x}}}{{(2\pi)}^{3/2}} f(\vb*{x}) e^{-i\vb*{k}\cdot\vb*{x}} \\
		%
		f(\vb*{x}) & = \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} f(\vb*{k}) e^{i\vb*{k}\cdot\vb*{x}}.
	\end{align}
\end{subequations}
%
Note for real fields \(f(\vb*{k}) = \conj{f}(-\vb*{k})\).
Here, \(\mathcal{P}_{f}(k)\) is the \emph{dimensionless power spectrum}.
It depends solely on \(k = \abs{\vb*{k}}\) due to statistical isotropy.
The Dirac delta function appears because of statistical homogeneity, the correlator has to be invariant under the active translation of the field --- a transformation in Fourier space \(f(\vb*{k}) \rightarrow f(\vb*{k}) e^{-i\vb*{k}\cdot\vb*{a}}\).
For the three-point correlator
%
\begin{equation}
	\expval*{f(\vb*{k}_{1}) f(\vb*{k}_{2}) f(\vb*{k}_{3})} \propto B_{f}(k_{1},k_{2},k_{3}) \delta^{3}(\vb*{k}_{1} + \vb*{k}_{2} + \vb*{k}_{3})
\end{equation}
%
where \(B_{f}(k_{1},k_{2},k_{3})\) is the \emph{bispectrum} of \(f\), which is a symmetric function of the three wavenumbers.
...
The three-point function vanishes for zero-mean \emph{Gaussian random fields} and so provides a good test of theories, \ie{} straightforward inflationary models which predict near-Gaussian primordial perturbations.
...
%
\begin{equation}\label{eq:two_point_rotate}
	\expval*{\harmonic{f} \conj{\harmonic[']{f}}} = \sum\limits_{n n'} \wigner{\ell}{m}{n} \wigner[\ast]{\ell}{m'}{n'} \expval*{f_{\ell n} \conj{f_{\ell' n'}}}.
\end{equation}
%
This holds if
%
\begin{equation}
	\expval*{f_{\ell n} \conj{f_{\ell' n'}}} = C_{\ell} \delta_{\ell \ell'} \delta_{m m'}
\end{equation}
%
since \cref{eq:two_point_rotate} is equal to
%
\begin{equation}
	\sum\limits_{n n'} \wigner{\ell}{m}{n} \wigner[\ast]{\ell}{m'}{n'} C_{\ell} \delta_{\ell \ell'} \delta_{m m'} = C_{\ell} \delta_{\ell \ell'} \sum\limits_{n} \wigner{\ell}{m}{n} \wigner[\ast]{\ell}{m'}{n'}.
\end{equation}
%
The real quantity \(C_{\ell}\) is the \emph{angular power spectrum} of \(\pixel{f}\).
...
%
\begin{align}
	\expval*{\pixel{f} \pixel[']{\conj{f}}}
	%
	 & = \harmonicSum \harmonicSum['] \expval*{\harmonic{f} \conj{\harmonic[']{f}}} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic[']{Y}}} \nonumber{} \\
	%
	 & = \sum\limits_{\ell} C_{\ell} \sum\limits_{m} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic{Y}}} \nonumber{}                                   \\
	%
	 & = \sum\limits_{\ell} \factor C_{\ell} P_{\ell}(\omega\cdot\omega')  \nonumber{}                                                                 \\
	%
	 & = C(\theta)
\end{align}
%
where \({\omega\cdot\omega' = \cos{\theta}}\), and using the spherical harmonic addition theorem
%
\begin{equation}
	\sum\limits_{m} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic{Y}}} = \factor P_{\ell}(\omega\cdot\omega').
\end{equation}
%
...
%
\begin{equation}
	C(0) = \sum\limits_{\ell} \factor C_{\ell} \approx \int \dd{\ln{\ell}} \frac{\ell(\ell+1) C_{\ell}}{2\pi}.
\end{equation}
%
Inverting the correlation function retrieves the power spectrum using the orthogonality of the Legendre polynomials
%
\begin{equation}
	C_{\ell} = 2\pi \int\limits_{-1}^{1} \dd{\cos{\theta}} \pixel{C} P_{\ell}(\cos{\theta}).
\end{equation}

\subsubsection{Projection of 3D Random Fields}
The CMB decoupled from matter around recombination.
The temperature anisotropies observed are the projection of the conditions at that time over the surface of \emph{last scattering}.
...
Expanding \(F(\vb*{x})\) in Fourier space
%
\begin{align}
	f(\vu{n})
	%
	 & = \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} F(\vb*{k}) e^{ikr \vu{k}\cdot\vu{n}} \nonumber{}                                                                \\
	%
	 & = 4\pi \harmonicSum i^{\ell} \bigg(\int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} F(\vb*{k}) j_{\ell}(kr) \conj{\harmonic{Y}}(\vu{k})\bigg) \harmonic{Y}(\vu{n})
\end{align}
%
where one uses \emph{Rayleigh plane-wave expansion}
%
\begin{equation}
	e^{i\vb*{k}\cdot\vb*{x}} = 4\pi \harmonicSum i^{\ell} j_{\ell}(kr) \conj{\harmonic{Y}}(\vu{k}) \harmonic{Y}(\vu{n}),
\end{equation}
%
and \(r = \abs{\vb*{x}}\) and \(j_{l}(kr)\) are the \emph{spherical Bessel functions}.
The spherical multipole coefficients of \(f(\vu{n})\) are
%
\begin{equation}
	\harmonic{f} = 4\pi i^{\ell} \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} F(\vb*{k}) j_{\ell}(kr) \conj{\harmonic{Y}}(\vu{k}).
\end{equation}
%
...
\begin{align}
	...
	%
	 & = {(4\pi)}^{2} i^{\ell-\ell'} \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} \frac{\dd[3]{\vb*{k'}}}{{(2\pi)}^{3/2}} \expval*{F(\vb*{k}) \conj{F}(\vb*{k'})} j_{\ell}(kr) j_{\ell'}(k'r) \conj{\harmonic{Y}}(\vu{k}) \harmonic[']{Y}(\vu{k'}) \nonumber{} \\
	%
	 & = {(4\pi)}^{2} i^{\ell-\ell'} \bigg(\int \dd{\ln{k}} \mathcal{P}_{F}(k) j_{\ell}(kr) j_{\ell'}(kr)\bigg) \bigg(\int \dd{\vu{k}} \conj{\harmonic{Y}}(\vu{k}) \harmonic[']{Y}(\vu{k})\bigg) \nonumber{}                                                  \\
	%
	 & = 4\pi \delta_{\ell \ell'} \delta_{m m'} \int \dd{\ln{k}} \mathcal{P}_{F}(k) j^{2}_{\ell}(kr).
\end{align}
%
Thus, \(f(\vu{n})\) is a statistically isotropic field with angular power spectrum
%
\begin{equation}
	C_{\ell} = 4\pi \int \dd{\ln{k}} \mathcal{P}_{F}(k) j^{2}_{\ell}(kr).
\end{equation}
%
...
\emph{Cosmic variance} is the random error which contributes to the error budget of most inferences made in observational cosmology.
...
Using the spherical multipoles \(\harmonic{f}\) forms the following estimator for \(C_{\ell}\)
%
\begin{equation}
	\hat{C}_{\ell} = \frac{1}{2\ell+1} \sum\limits_{m} \harmonic{f} \conj{\harmonic{f}}.
\end{equation}
%
This unbiased estimator, \ie{} \(\expval*{\hat{C}_{\ell}} = C_{\ell}\), and the sum over \(m\) means for a given \({\ell}\) one uses all the available information in the \(2\ell+1\) independent degrees of freedom in the multipoles --- however, this estimator has non-zero variance.
Using Wick's theorem one calculates the variance of a Gaussian random field
%
\begin{align}
	\operatorname{Var}(\hat{C}_{\ell})
	%
	 & = \expval*{\hat{C}_{\ell}\hat{C}_{\ell}} - \expval*{\hat{C}_{\ell}}^{2} \nonumber{}                                                                                                                                                             \\
	%
	 & = \frac{1}{{(2\ell+1)}^{2}} \sum\limits_{m m'} \expval*{\harmonic{f} \conj{\harmonic{f}} f_{\ell m'} \conj{f_{\ell m'}}} - C_{\ell}^{2} \nonumber{}                                                                                             \\
	%
	 & = \frac{1}{{(2\ell+1)}^{2}} \sum\limits_{m m'} \Big(\expval*{\harmonic{f} f_{\ell m'}} \expval*{ \conj{\harmonic{f}} \conj{f_{\ell m'}}} + \expval*{\harmonic{f} \conj{f_{\ell m'}}} \expval*{\conj{\harmonic{f}} f_{\ell m'}}\Big) \nonumber{} \\
	%

...
\begin{equation}
	...
	%
	= {(2\pi)}^{3} \powerSpectrum{f} \delta^{3}(\vb*{k} - \vb*{k'})
\end{equation}
%
...
\end{equation}
%
Here, \(\powerSpectrum{f}\) is the dimensionless power spectrum.
...
\begin{equation}
	...
	%
	= \wignerSum \wignerSum['] \wigner{\ell}{m}{n} \wigner[\ast]{\ell'}{m'}{n'} \expval*{f_{\ell n} \conj{f_{\ell' n'}}}.
\end{equation}
%
This only holds if
%
...
\begin{align}
	...
	&= \wignerSum \wignerSum['] \wigner{\ell}{m}{n} \wigner[\ast]{\ell'}{m'}{n'} \expval*{f_{\ell n} \conj{f_{\ell' n'}}} \nonumber{} \\
	%
	&= \wignerSum \wignerSum['] \wigner{\ell}{m}{n} \wigner[\ast]{\ell'}{m'}{n'} C_{\ell} \delta_{\ell \ell'} \delta_{n n'} \nonumber{} \\
	%
	&= C_{\ell} \delta_{\ell\ell'} \wignerSum \wigner{\ell}{m}{n} \wigner[\ast]{\ell'}{m'}{n} \nonumber{} \\
	%
	&= C_{\ell} \delta_{\ell\ell'} \delta_{m m'}.
\end{align}
%
The real quantity \(C_{\ell}\) is the angular power spectrum of \(\pixel{f}\).
...
\begin{align}
	...
	%
	 & = \sum\limits_{\ell=0}^{\infty} \sum\limits_{m=-\ell}^{\ell} \sum\limits_{\ell'=0}^{\infty} \sum\limits_{m'=-\ell'}^{\ell'} \expval*{\harmonic{f} \conj{\harmonic[']{f}}} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic[']{Y}}} \nonumber{} \\
	 %
	 &= \sum\limits_{\ell=0}^{\infty} \sum\limits_{m=-\ell}^{\ell} \sum\limits_{\ell'=0}^{\infty} \sum\limits_{m'=-\ell'}^{\ell'} C_{\ell} \delta_{\ell\ell'} \delta_{m m'} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic[']{Y}}} \nonumber{} \\
	%
	 & = \sum\limits_{\ell=0}^{\infty} C_{\ell} \sum\limits_{m=-\ell}^{\ell} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic{Y}}} \nonumber{}                                   \\
	%
	 & = \sum\limits_{\ell=0}^{\infty} \factor C_{\ell} P_{\ell}(\omega\cdot\omega')  \nonumber{}                                                                 \\
	%
	 & = C(\theta)
\end{align}
%
where \({\omega\cdot\omega' = \cos{\theta}}\), and using the spherical harmonic addition theorem \cref{eq:chapter2_addition_theorem}.
%
...
\begin{equation}
	C(0)
	%
	= \sum\limits_{\ell=0}^{\infty} \factor C_{\ell}
	%
	\approx \int \dd{\ln{\ell}} \frac{\ell(\ell+1) C_{\ell}}{2\pi}.
\end{equation}
%
...
\begin{align}
	f(\vu{n})
	%
	 & = \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} \fourier{f}(\vb*{k}) \exp(ikr \vu{k}\cdot\vu{n}) \nonumber{}                                                                \\
	%
	 & = 4\pi \sum\limits_{\ell=0}^{\infty} \sum\limits_{m=-\ell}^{\ell} i^{\ell} \bigg(\int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} \fourier{f}(\vb*{k}) j_{\ell}(kr) \conj{\harmonic{Y}}(\vu{k})\bigg) \harmonic{Y}(\vu{n})
\end{align}
%
where one uses Rayleigh plane-wave expansion
%
\begin{equation}
	\exp(i\vb*{k}\cdot\vb*{x})
	%
	= 4\pi \sum\limits_{\ell=0}^{\infty} \sum\limits_{m=-\ell}^{\ell} i^{\ell} j_{\ell}(kr) \conj{\harmonic{Y}}(\vu{k}) \harmonic{Y}(\vu{n}),
\end{equation}
%
and \(r = \abs{\vb*{x}}\) and \(j_{l}(kr)\) are the spherical Bessel functions.
The spherical multipole coefficients of \(f(\vu{n})\) are
%
\begin{equation}
	\harmonic{f}
	%
	= 4\pi i^{\ell} \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} \fourier{f}(\vb*{k}) j_{\ell}(kr) \conj{\harmonic{Y}}(\vu{k}).
\end{equation}
%
...
\begin{align}
	\expval*{\harmonic{f} \conj{\harmonic[']{f}}}
	%
	 & = {(4\pi)}^{2} i^{\ell-\ell'} \int \frac{\dd[3]{\vb*{k}}}{{(2\pi)}^{3/2}} \frac{\dd[3]{\vb*{k'}}}{{(2\pi)}^{3/2}} \expval*{\fourier{f}(\vb*{k}) \conj{\fourier{f}}(\vb*{k'})} j_{\ell}(kr) j_{\ell'}(k'r) \conj{\harmonic{Y}}(\vu{k}) \harmonic[']{Y}(\vu{k'}) \nonumber{} \\
	%
...
\begin{equation}
	C_{\ell}
	%
	= 4\pi \int \dd{\ln{k}} \powerSpectrum(F) j^{2}_{\ell}(kr).
\end{equation}
%

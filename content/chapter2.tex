...
%
\begin{equation}\label{eq:chapter2_reduced_wigner}
	\wigner{\ell}{m'}{m}(\rho)
	%
...
The \(\reducedWigner{\ell}{m'}{m}(\beta)\) are the \emph{reduced d matrices}, which are even functions and real in this phase convention.
...
\end{align}
%
where the last line follows by (\cf{} \cref{eq:chapter2_reduced_wigner})
%
\begin{align}
	\wigner{\ell}{m'}{m} (\rho^{-1})
	%
	 & = \exp(-im'(-\gamma)) \reducedWigner{\ell}{m'}{m}(-\beta) \exp(-im(-\alpha)) \nonumber{}           \\
	%
	 & = \conj{\big(\exp(-im\alpha) \reducedWigner{\ell}{m}{m'}(\beta) \exp(-im'\gamma)\big)} \nonumber{} \\
	%
...
\cref{fig:chapter2_harmonic_gaussian} shows both an elongated (top-left panel) and symmetric (bottom-left panel) harmonic Gaussian on the north pole, with the result of the translation in the top-right and bottom-right panels respectively.

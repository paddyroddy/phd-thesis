...
\subsection{Spherical Harmonics}\label{sec:chapter2_spherical_harmonics}
...
Rotations of functions defined on the sphere form an integral part of analyses on the sphere.
...
\input{figures/chapter2/rotation_sphere.tex}
...
In practice, the CWT described in \cref{sec:chapter2_continuous_wavelet_transform} is impractical to use for several reasons.
...
Here, the parameter combination \(\lambda=3\), \(J_{0}=2\) and bandlimit \(L=128\) results in a scaling function in panel (a) and four wavelets shown in panels (b--e). % chktex 8
...
Starting from the left of \cref{fig:chapter2_power_spectrum}, the first obvious feature is the peak at \(\ell \almost{200}\) (\ie{} an angular scale of \(\almost{1}\degree{}\)).
...
Experimentally, the Universe is spatially flat to a \(1\sigma{}\) accuracy of \(2\%\)~\cite{Planck2020b}. % chktex 21

To understand the second peak, consider a potential well with masses and springs.
...
The ratio of the second to the first peak amplitudes gives a measure of the baryon fraction of the critical density of the Universe.

At early times, the Universe is radiation dominated, and the radiation pressure causes the gravitational potential to decay.
...
The enhanced third peak is a sign that the matter density was dark matter dominated in the plasma before recombination.
Finally, at high \(\ell{}\) (smaller angular scales) the angular coherence of the temperature fluctuations drop off --- leading to a damping tail.
Approaching the epoch of decoupling (\ie{} \(z \almost{1100}\)), the coupling between photons and baryons deteriorates; the photon mean free path increases, and they diffuse and smooth out temperature anisotropies on physical scales comparable to the distance travelled.
Thus, the acoustic oscillations are exceptionally damped on scales smaller than photon random walk distance during the epochs of recombination and decoupling.
This damping tail may be used to verify assumptions about recombination and the initial conditions.
...
\subsubsection{General Analysis}\label{sec:chapter2_general_analysis}

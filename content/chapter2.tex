...
In these time-frequency joint representations, the idea is to cut the signal into multiple parts and analyse them independently, which may give more information on the frequency components.
Although, making a suitable cut is non-trivial due to the Heisenberg uncertainty problem, which, in signal processing terms, states that it is impossible to know both the exact frequency, and the exact time of occurrence of this frequency in the signal.
...
In wavelet analysis, one typically refers to time-scale representations, as opposed to time-frequency (like in Fourier theory), where scale is effectively the opposite of frequency.
\cref{sec:chapter2_wavelets_euclidean} follows with some notable examples of wavelets in the Euclidean setting, some spherical analogues are then discussed in \cref{sec:chapter2_wavelets_sphere}.

\subsection{Euclidean}\label{sec:chapter2_wavelets_euclidean}

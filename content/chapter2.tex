...
\end{equation}
%
where \(j\) and \(k\) are integers, \(s_{0}>1\) is a fixed dilation step, and \(\tau_{0}\) is the translation factor (typically \(\tau_{0}=1\) for dyadic sampling).
...
A scaling function \(\Phi_{k}(t)\), in blue, captures the lower scales of the underlying function --- introduced by~\cite{Mallat1989}.

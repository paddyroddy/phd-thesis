...
The Shannon number for this region and bandlimit is \(N=30\), and therefore all Slepian functions shown are well-concentrated within the region --- as shown by the eigenvalues \(\mu_{p}\) being \(\almost{1}\).
Visually, these polar cap Slepian functions look similar to the spherical harmonics (\cf{} \cref{fig:chapter2_spherical_harmonics}); where the number of positive/negative regions relates to the order \(m\) in the submatrix \(D_{\ell m,\ell'm}\).

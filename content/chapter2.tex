...
\end{equation}
%
This example region often occurs in geodesy, where satellites map out gravitational or magnetic fields of planets whilst flying on inclined polar orbits --- leaving behind a gap between two antipodal axisymmetric polar caps~\cite{Simons2006a}.
...
In non-Gaussianity analysis of the CMB, the so-called Kp0 exclusion mask is applied to the data~\cite{Vielva2004,McEwen2004}; however, this contaminates the wavelet coefficients that overlap with the mask.
Thus, the contaminated wavelet coefficients must be removed before any analysis.
The solutions of the Slepian concentration problem are the basis functions of the region (of the sphere); hence, wavelets may be constructed on these basis functions outside the masked region in \cref{fig:chapter2_planck_masked}.

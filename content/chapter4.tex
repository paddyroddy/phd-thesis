...
By generalising the \emph{sifting convolution}~\cite{Roddy2021}, discussed in \cref{sec:chapter3}, one may perform convolutions over the incomplete sphere using the Slepian functions as a basis.
...
\begin{equation}
	\pixel{f}
	%
	= \displaystyle\sum\limits_{\ell=0}^{\infty} \displaystyle\sum\limits_{m=-\ell}^{\ell} \harmonic{f} \pixel{\harmonic{Y}},
\end{equation}
%
...
\begin{equation}
	\harmonicSum
	%
	= \displaystyle\sum\limits_{\ell=0}^{L-1} \displaystyle\sum\limits_{m=-\ell}^{\ell}
\end{equation}
%
may be adopted.
...
\begin{equation}\label{eq:chapter4_addition_theorem}
	\displaystyle\sum\limits_{m=-\ell}^{\ell} \pixel{\harmonic{Y}} \pixel[']{\conj{\harmonic{Y}}}
	%
...
\begin{equation}
	...
	%
	= \displaystyle\sum\limits_{m'=-\ell}^{\ell} \wigner{\ell}{m'}{m}(\rho) f_{\ell m'},
\end{equation}
%
...
\input{figures/chapter4/example_region_sphere}
...
\begin{align}\label{eq:chapter4_shannon}
	...
	%
	 & = \displaystyle\sum\limits_{\ell} \factor P_{\ell}(1) \integrateRegion{\sphereVolume} \nonumber                        \\
	%
	 & = \frac{A}{4\pi} L^{2},
\end{align}
%
...
\begin{equation}
	\pixel{f}
	%
	= \displaystyle\sum\limits_{p=1}^{L^{2}} \slepian{f}\pixel{\slepian{S}}.
\end{equation}
%
...
\begin{equation}\label{eq:chapter4_slepian_inverse}
	\pixel{f}
	%
	\approx \displaystyle\sum\limits_{p=1}^{N} \slepian{f}\pixel{\slepian{S}} \\
	%
...
\input{figures/chapter4/south_america_tiling}
...
To simplify \cref{eq:chapter4_slepian_isotropic_noise} further, consider homogenous and isotropic noise defined by its power spectrum
%
\begin{equation}
...
\input{figures/chapter4/south_america_region}
...
\input{figures/chapter4/south_america_slepian_functions}

\input{figures/chapter4/south_america_eigenvalues}
...
\input{figures/chapter4/south_america_wavelets}

\input{figures/chapter4/south_america_wavelet_coefficients}
...
\input{figures/chapter4/south_america_denoising}

\section{Conclusions}\label{sec:chapter4_conclusion}

This chapter presents the construction of \emph{Slepian scale-discretised wavelets on the sphere}, which are wavelets restricted to a region of the sphere.
...
The current chapter generalises the \emph{sifting convolution}~\cite{Roddy2021} on the sphere (introduced in \cref{sec:chapter3}) beyond the spherical harmonic setting to any basis, where here the Slepian functions define the basis.

...
A manifold is sampled at \(n\) points, and a graph is then constructed on these points (vertices) where the edges represent the local connectivity of the manifold.
...
For triangular meshes, the most straightforward discretisation of the Riemannian metric is performed by assigning each edge a length \(l_{i j}>0\).
The mesh Laplacian is then found by substituting
%
\begin{equation}\label{eq:chapter4_mesh_laplace_weights}
	w_{i j}
	%
	= \frac{-l_{i j}^{2} + l_{j k}^{2} + l_{i k}^{2}}{8 a_{ijk}}
	%
	+ \frac{-l_{i j}^{2} + l_{j h}^{2} + l_{i h}^{2}}{8 a_{ijh}}
\end{equation}
%
and
%
\begin{equation}
	a_{i}
	%
	= \frac{1}{3} \sum\limits_{j k : (i,j,k) \in \faces} a_{ijk}
\end{equation}
%
...
\begin{equation}
	a_{ijk}
	%
	= \sqrt{s_{ijk}(s_{ijk}-l_{ij})(s_{ijk}-l_{jk})(s_{ijk}-l_{ik})},
\end{equation}
%
...
\end{equation}
%
where \(\alpha_{i j}\) and \(\beta_{i j}\) are the internal angles opposite to the corresponding edge.

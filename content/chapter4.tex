...
Slepian wavelets are constructed on a region of a manifold, and are therefore suited to problems where data only exists, or are missing in a particular region.
...
One manifold, in particular, which is in commonplace in science and engineering is the sphere; such as in: cosmology~\autocite{Bennett1996}, geophysics~\autocite{Simons2006}, planetary science~\autocite{Turcotte1981}, computer graphics~\autocite{Ramamoorthi2004}, and signal processing~\autocite{Roddy2021a}.
...
One such example, in the spherical setting, is in cosmic microwave background analyses where foreground microwave emissions dominate the region around the  Galactic plane and are often removed~\autocite{Mortlock2002}.
...
Wavelets on the sphere (\cf{} manifold) have been applied in fields such as: astrophysics and cosmology~\autocite{Pen1999,Barreiro2001,Rocha2004,McEwen2004}, planetary science~\autocite{Audet2011,Audet2014}, and geophysics~\autocite{Loris2010,Simons2011,Simons2011b}.
Scale-discretised wavelets (on the sphere)~\autocite{Wiaux2008,McEwen2018,Leistedt2013,McEwen2013,McEwen2015} lean on a tiling of the harmonic line to produce an exact wavelet transform in both the continuous and discrete settings.

A new research direction, \emph{geometric deep learning}, has evolved from an increasing interest in non-Euclidean data geometries; where convolutional networks are generalised to graph and manifold structured data (\eg{}~\autocite{Bronstein2017,Perlmutter2020}).
...
Manifold learning models (\eg{}~\autocite{Tenenbaum2000,Coifman2006b,VanDerMaaten2008}) are a set of unsupervised algorithms which capture the intrinsic structure in the data through data-driven geometries.
Moreover, signals on manifolds are increasingly prevalent in areas such as shape matching and computer graphics.
Thus, many works are emerging to generalise spectral and signal processing methods to manifolds~\autocite{Coifman2006} and graphs~\autocite{Shuman2013}.
...
Graphs are a fundamental data structure, which are useful in describing geometric structures of data in domains such as: social~\autocite{Nettleton2013}, transportation~\autocite{Mohan2014}, sensor~\autocite{Kenniche2010}, and neuronal~\autocite{Tang2012} networks.
...
It is well-known that functions cannot have finite support in the spatial and spectral domains simultaneously~\autocite{Slepian1961,Slepian1983}.
In the 1960s, Slepian, Landau and Pollak solved the fundamental problem of finding and representing signals which are optimally concentrated in both domains~\autocite{Slepian1961,Landau1961,Landau1962}.
...
The Slepian concentration problem was initially formed in the Euclidean domain; however, it has since been generalised to other geometries, such as: the sphere~\autocite{Simons2006,Wieczorek2005,Albertella1999,Cohen1989,Meaney1984,Daubechies1988}, and graphs~\autocite{VanDeVille2017,VanDeVille2017a,Bolton2018}.
The so-called \emph{Slepian functions} arise in many fields within science and engineering, such as: medical imaging~\autocite{Jackson1991}, signal processing~\autocite{Mathews1985,Thomson1982}, and geophysics~\autocite{Thomson1976,Simons2006a,Simons2011}.
Notably, these functions have been used in interpolation~\autocite{Moore2004,Shkolnisky2006}, extrapolation~\autocite{Xu1983}, inverse problems~\autocite{Villiers2001,Abdelmoula2015}, and solving partial differential equations~\autocite{Boyd2003,Chen2005}.

Due to recent interest in geometric deep learning, many fields are extending existing methods developed in the Euclidean domain to manifolds or graphs, \ie{}~\autocite{Perlmutter2020}.
This work generalises Slepian scale-discretised wavelets on the sphere~\autocite{Roddy2021a} to Riemannian manifolds.
These wavelets constitute a basis designed for incomplete data on manifolds or graphs.
Here, the scale-discretised wavelet construction on the sphere~\autocite{Wiaux2008,McEwen2018,Leistedt2013,McEwen2013,McEwen2015} is extended to the Slepian domain; however, the Slepian functions are now constructed on a region of a general manifold (rather than strictly the sphere).
...
The wavelet transform follows through a generalisation of the \emph{sifting convolution}~\autocite{Roddy2021}, which allows one to perform convolutions on (incomplete) manifolds.
...
Plainly, wavelets in the Slepian domain have been considered before --- this work generalises prior work on the sphere to general manifolds~\autocite{Roddy2021a}.
Multiresolution analyses have been performed with prolate spheroidal wave functions~\autocite{Walter2004} in the Euclidean setting.
In spherical settings, spatially localised spherical harmonic transforms are sometimes used (\eg{}~\autocite{Simons1997,Wieczorek2005,Khalid2013,Khalid2013a}).
Other times, the Slepian functions on the sphere are used directly~\autocite{Simons2009}.
A variety of other approaches include: established regularisation techniques based on a known singular value decomposition~\autocite{Michel2017}, and wavelet-like representations without explicit inverse transforms~\autocite{Simons2011}.
Extending wavelets and wavelet techniques to manifold and graph domains has been extensively reviewed (\eg{}~\autocite{Dahmen1999,Coifman2006a}).
...
A brief review of Slepian scale-discretised wavelets on the sphere~\autocite{Roddy2021a} follows, which this works builds upon.
...
A review of the sifting convolution~\autocite{Roddy2021} follows, which allows one to perform convolutions on the (incomplete) manifold --- which are typically not well-defined.
...
A numerical illustration is presented in \cref{sec:chapter4_numerical_illustration}, in which the Slepian functions of the head region of a triangular mesh of Homer Simpson are computed.
...
\end{equation}
%
which converges to the Laplace-Beltrami operator as the number of samples goes to infinity~\autocite{Belkin2007}.
...
The \emph{non-normalised graph Laplacian} refers to setting \(\vb{A} = \vb{I}\) in \cref{eq:chapter4_graph_laplace_matrix}; other choices exist, such as the random walk Laplacian which occurs when \(\vb{A} = \vb{K}\)~\autocite{VonLuxburg2007}.

\subsubsection{Triangular Meshes}

In computer graphics applications, three-dimensional shapes are often modelled by two-dimensional manifolds.
...
\end{equation}
%
However, this discretisation does not capture the geometry of continuous underlying manifold; hence, the graph Laplacian would not converge to the Laplace-Beltrami operator at the limit of many of samples~\autocite{Wardetzky2007}.
...
%
\begin{equation}\label{eq:chapter4_mesh_laplace_weights}
	w_{i j}
	%
...
\end{equation}
%
into \cref{eq:chapter4_graph_laplace_indices}, where the area of a triangle is
%
\begin{equation}
	a_{i j k}
	%
...
\end{equation}
%
is induced by an embedding of the mesh, which changes \cref{eq:chapter4_mesh_laplace_weights} to
%
\begin{equation}
	w_{i j}
	%
...
\end{equation}
%
where \(\alpha_{i j}\) and \(\beta_{i j}\) are the internal angles opposite to the corresponding edge \(\ell_{i j}\) (\cf{} \cref{fig:chapter4_mesh_laplace}).
This \emph{cotangent Laplacian} is used extensively in computer graphics~\autocite{Pinkall1993}, and in the rest of this work.
A demonstration of the mesh Laplacian eigenvectors for a given mesh is presented in \cref{fig:chapter4_eigenhomers}.

\input{figures/chapter4/mesh_laplace}

\input{figures/chapter4/eigenhomers}

\subsection{Problem Formulation}\label{sec:chapter4_problem_formulation}

This work generalises Slepian scale-discretised wavelets on the sphere~\autocite{Roddy2021a} to arbitrary manifolds.
...
In the aforesaid work, the wavelets are constructed through a tiling of the Slepian line (akin to the harmonic line) in an analogous manner to~\autocite{Wiaux2008,McEwen2018}.
The sifting convolution~\autocite{Roddy2021} allows one to perform convolutions where both inputs are directional, whilst the output remains on the sphere.
...
The sifting convolution~\autocite{Roddy2021} is then defined similarly, allowing for wavelet transforms on the manifold to be constructed.
...
The problem of spatial concentration of bandlimited signals (likewise spectral concentration of spacelimited signals) was first studied by Slepian, Landau and Pollak in the 1960s~\autocite{Slepian1961,Landau1961,Landau1962}.
Initially considered for time domain signals, it has since been extended to other domains such as signals on the sphere~\autocite{Simons2006,Roddy2021a,Xu1983,Wieczorek2005}.
...
\cref{eq:chapter4_spatial_concentration_ratio} can be rewritten as a matrix variational problem
%
\begin{equation}
	\mu
	%
...
The Shannon number is a good estimate of the number of significant eigenvalues~\autocite{Percival1993}.
...
\end{equation}
%
where \(\vb{S}\) are the harmonic coefficients of \(\mesh{\slepian{S}}\) (see \cref{eq:chapter3_integrate_region_proof} for proof).
...
Recently developed by the authors of this work, the sifting convolution~\autocite{Roddy2021} is built on a translation which simply involves a product of the basis functions in Fourier space.
...
Initially, \cref{sec:chapter4_slepian_sifting_convolution} extends the sifting convolution~\autocite{Roddy2021} to the Slepian domain.
...
The construction here is the same as before in \cref{sec:chapter3_slepian_wavelets}, where now the integrals are over the manifold (graph) rather than strictly the sphere.
...
The sifting convolution~\autocite{Roddy2021} on the manifold, defined in \cref{sec:chapter4_slepian_concentration_problem_manifolds}, can be extended to work with the Slepian functions as a basis.
...
\end{equation}
%
which becomes a product in Slepian space, and hence is efficient to compute.
...
The construction of these wavelets is in an analogous manner to~\autocite{Wiaux2008,McEwen2018} but are computed in the Slepian basis rather than the basis functions of the sphere (\cf{} manifold).
...
A tiling of the Slepian line defines the wavelets through smooth generating functions --- introduced in \cref{sec:chapter3_generating_functions}.
Defined in~\autocite{Wiaux2008}, \(k_{\lambda}\) is a smoothly decreasing function which is unity for \(t < 1/\lambda{}\), zero for \(t > 1\), and smoothly decreasing otherwise.
...
These properties are the same as the spherical case, and are extensively discussed in \cref{sec:chapter3_properties}.
...
In contrast, Slepian wavelets are built on the Slepian harmonic line, and thus \(j\) is a measure of localisation rather than frequency.
...
In this section the construction and application of Slepian wavelets for an example region on a mesh (\cf{} manifold) is demonstrated, as before in \cref{sec:chapter3_numerical_illustration}.
...
All computations are performed with the \texttt{S2LET}\footnote{\url{http://astro-informatics.github.io/s2let/}}~\autocite{Leistedt2013} code, which enables the construction of the wavelet generating functions discussed in \cref{sec:chapter3_generating_functions}.
...
\cref{fig:chapter4_homer_data} presents such a signal, the \(z\)-component of the per vertex normals of the Homer mesh.
...
To showcase Slepian wavelets, white noise is added to the signal in \cref{fig:chapter4_homer_data}.
...
\end{equation}
%
A denoised version of \(z\) is desired, denoted by \(d \in \hilbert{R}\), with a large \(\snr{d}\) (defined in \cref{eq:chapter3_signal_to_noise}) such that \(d\) extracts the informative signal \(s\).
As before, the scaling function and wavelets are treated equivalently in the Slepian setting.
The scaling function in the Slepian setting is not a low-frequency representation of the signal due to the localisation of the Slepian functions (see \cref{sec:chapter3_localisation}).
...
To perform the denoising, Gaussian white noise is added to the data in panel (a) of \cref{fig:chapter4_denoising}.
...
This work generalises the recently developed \emph{Slepian scale-discretised wavelets on the sphere}~\autocite{Roddy2021a} to Riemannian manifolds, whereby the wavelets themselves are restricted to a region of the manifold.
Slepian wavelets have many possible use cases in various fields of science and engineering, where data only exists, or are missing in a particular region.
...
The wavelet transform is built on the sifting convolution~\autocite{Roddy2021} which allows one to perform convolutions on (incomplete) manifolds --- which, in general, are not well-defined.
...
As before, Slepian wavelets on manifolds can be used many standard wavelet applications; for example, solving inverse problems via sparse regularisation~\autocite{McEwen2013a,Wallis2017,Price2021}.

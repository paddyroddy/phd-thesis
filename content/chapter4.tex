...
In this field, 3D shapes are often modelled as Riemannian manifolds and discretised as triangular meshes.
...
In computer graphics applications, 3D shapes are often modelled by 2D manifolds.
...
\end{equation}
%
However, this discretisation does not capture the geometry of continuous underlying manifold; hence, the graph Laplacian would not converge to the Laplace-Beltrami operator at the limit of many of samples~\cite{Wardetzky2007}.
...
This \emph{cotangent Laplacian} is used extensively in computer graphics~\cite{Pinkall1993}, and in the rest of this work.
...
Recently developed by the authors of this work, the sifting convolution~\cite{Roddy2021} is built on a translation which simply involves a product of the basis functions in Fourier space.
...
The sifting convolution~\cite{Roddy2021} on the manifold, defined in \cref{sec:chapter4_slepian_concentration_problem_manifolds}, can be extended to work with the Slepian functions as a basis.
...
Defined in~\cite{Wiaux2008}, \(k_{\lambda}\) is a smoothly decreasing function which is unity for \(t < 1/\lambda{}\), zero for \(t > 1\), and smoothly decreasing otherwise.
...
All computations are performed with the \texttt{S2LET}\footnote{\url{http://astro-informatics.github.io/s2let/}}~\cite{Leistedt2013} code, which enables the construction of the wavelet generating functions discussed in \cref{sec:chapter3_generating_functions}.
...
This work generalises the recently developed \emph{Slepian scale-discretised wavelets on the sphere}~\cite{Roddy2021a} to Riemannian manifolds, whereby the wavelets themselves are restricted to a region of the manifold.
...
In computer graphics applications, 3D shapes are often modelled as 2D manifolds, which are then discretised as meshes.
...
As before, Slepian wavelets on manifolds can be used many standard wavelet applications; for example, solving inverse problems via sparse regularisation~\cite{McEwen2013a,Wallis2017,Price2021}.

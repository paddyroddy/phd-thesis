...
\hl{The mathematical preliminaries assumed for this chapter are covered in \mbox{\cref{sec:chapter2_notation,sec:chapter2_spherical_harmonics,sec:chapter2_rotations_sphere}}}.
...
\hl{This section expands on the work in \mbox{\cref{sec:chapter2_spatial_concentration_bandlimited_function}}.
For a bandlimited function \mbox{\(f \in \hilbert{\twoSphere}\)} within a region \mbox{\(R\)}, one may express \mbox{\cref{eq:chapter2_harmonic_eigenproblem}} in matrix form by}
%
...
Note the use of the orthogonality results \hl{in}
%
...
The corresponding eigenvalues are shown in \cref{fig:chapter4_eigenvalues}, where the eigenvalues remain \(\almost{1}\) for many \(p\) values until decreasing towards zero.

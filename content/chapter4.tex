...
\end{equation}
%
No bandlimited function can be restricted perfectly within a region \(R\) and hence \(\mu_{1}<1\).
...
\begin{equation}
	N
	%
	= \frac{\integrateRegion{\meshVolume}}{\integrateManifold{x}} \imax,
\end{equation}
%
which is a good estimate of the number of significant eigenvalues ~\cite{Percival1993}.
...
%
\begin{equation}
	\mesh{(\translation{\vb{x'}}\phi_{i})}
	%
	\equiv \mesh[']{\phi_{i}} \mesh{\phi_{i}},
\end{equation}
%
where \(\vb{x'}\) is a point on the manifold.
An arbitrary function \(f \in \hilbert{\manifold}\) is translated thus
%
\begin{equation}
	\mesh{(\translation{\vb{x'}}f)}
	%
	= \sum\limits_{i} f_{i} \mesh[']{\phi_{i}} \mesh{\phi_{i}},
\end{equation}
%
implying
%
\begin{equation}
	{(\translation{\vb{x'}}f)}_{i}
	%
	= f_{i} \mesh[']{\phi_{i}}.
\end{equation}

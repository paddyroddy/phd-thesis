...
This work presents the construction of a novel spherical wavelet basis designed for incomplete spherical datasets, \ie{} datasets which are missing in a particular region.
...
Through a tiling of the Slepian harmonic line one may construct scale-discretised wavelets which lead to a theoretically exact wavelet transform.
...
The usual approach to deal with data of this form is to use wavelets, which allow one to probe spatially localised, scale-dependent features of signals on the sphere.
...
A possible approach to solve this problem is to construct wavelets which are concentrated in the region.
...
Wavelets on the sphere have been effectively applied in fields such as astrophysics and cosmology~\cite{Pen1999,Barreiro2001,Rocha2004,Mukherjee2003,McEwen2004a}, where datasets are increasingly large and require analysis at high resolutions for accurate theoretical predictions.
...
Scale-discretised wavelets~\cite{Wiaux2008,McEwen2018} lean on a tiling of the harmonic line to produce an exact wavelet transform in both the continuous and discrete settings.
...
Slepian, Landau and Pollak solved the fundamental problem of finding and representing the functions which are optimally energy concentrated in both the time and frequency domains~\cite{Slepian1961,Landau1961,Landau1962,Sonnenblick1965}.
...
The \emph{Slepian functions} and their multidimensional counterparts~\cite{Slepian1964,Simons2010} have been used in many branches of science and engineering (\eg{} signal processing~\cite{Mathews1985,Thomson1982}, geophysics~\cite{Thomson1976,Simons2006a,Simons2011}, medical imaging~\cite{Jackson1991}).
In particular, these functions have been used in solving partial differential equations~\cite{Boyd2003,Chen2005}, inverse problems~\cite{DeVilliers2001,Abdelmoula2015}, interpolation~\cite{Moore2004,Shkolnisky2006,Shkolnisky2007} and extrapolation~\cite{YuanXu1983}.
...
The initial formulation of the Slepian concentration problem was developed in the Euclidean domain; however, this has since been generalised to other geometries~\cite{Simons2006,Mortlock2002,Wieczorek2005,Albertella1999,Cohen1989,Meaney1984,Daubechies1988}.
...
The ensuing bandlimited, spatially concentrated functions have been applied for localised spectral analysis~\cite{Wieczorek2013}, and spectral estimation of signals~\cite{Dahlen2007} defined on the sphere.

There have been attempts to construct wavelet-like approaches or multiresolution algorithms~\cite{Michel2017} in the Slepian domain before.
However, there lacks a definition that has a theoretically exact wavelet transform (assuming accurate Slepian functions) in both the continuous and discrete settings.
Some methods use a dictionary in place of a full wavelet transform (\eg{}~\cite{Simons2011}).
Others exploit the wavelet-like behaviour of the eigenfunctions of the Slepian concentration problem directly~\cite{Lilly1995a,Walter2004,Walter2005,Walter2005a}.

This work presents a novel spherical wavelet basis designed for incomplete spherical datasets.
The eigenfunctions of the Slepian concentration problem are orthogonal basis functions (the Slepian functions) for the region from which they are constructed.
A tiling of the Slepian line --- akin to the harmonic line --- allows one to define scale-discretised wavelets which lead to a theoretically exact wavelet transform.
Through the \emph{sifting convolution}~\cite{Roddy2021} one may perform convolutions over the incomplete sphere using the Slepian functions as a basis.
\emph{Slepian scale-discretised wavelets on the sphere} or \emph{Slepian wavelets} for short have many possible applications wherever data are missing in a spherical setting; for example, in cosmological analyses.
...
The inner product between two functions \(f,\ g \in \hilbert{\twoSphere}\) is
%
\begin{equation}
	\braket*{f}{g}
	%
...
\end{equation}
%
where \(\dd{\pixel{\Omega}} = \sin{\theta} \dd{\theta} \dd{\phi}\) is the usual invariant measure on \(\twoSphere{}\).
...
The spherical harmonic functions \(\pixel{\harmonic{Y}}\) are defined for integer degree \(\ell \geq 0\) and integer order \(\abs{m} \leq \ell{}\) as
%
\begin{equation}
	\pixel{\harmonic{Y}}
	%
	= \sqrt{\factor \frac{(\ell-m)!}{(\ell+m)!}} P_{\ell}^{m} (\cos{\theta}) e^{im\phi},
\end{equation}
%
where \(P^{\ell}_{m}(x)\) are the \emph{associated Legendre polynomials}.
...
%
\begin{equation}
	\pixel{\conj{\harmonic{Y}}} = {(-1)}^{m} \pixel{Y_{\ell(-m)}},
\end{equation}
%
such that
%
\begin{equation}
	{\conj{\harmonic{f}} = {(-1)}^{m} f_{\ell(-m)}}
\end{equation}
%
for a real field.
...
%
\begin{equation}
	\harmonicSum = \sum\limits_{\ell=0}^{L-1} \sum\limits_{m=-\ell}^{\ell}
\end{equation}
%
may be adopted.
...
%
\begin{align}\label{eq:chapter3_sifting_convolution}
...
%
\begin{equation}\label{eq:chapter3_eigenvalues}
...
\begin{align}
	...
	%
	 & =  \slepianSum['] \slepian[']{f} \slepian[']{\mu} \slepian{\vb*{S}}^{\dagger} \slepian[']{\vb*{S}} \nonumber         \\
	%
...
This convolution is then used to define Slepian wavelets in \cref{sec:chapter3_slepian_scale_discretised_wavelets_sphere} with a suitable admissibility condition to ensure exact reconstruction.
The generating functions are constructed in \cref{sec:chapter3_generating_functions} which define the wavelets.
...
\begin{align}\label{eq:chapter3_slepian_wavelet_omega}
	...
	%
	 & = \braket*{\translation{\omega}\Psi^{j}}{f} \nonumber                                      \\
	%
...
\begin{align}\label{eq:chapter3_synthesis}
	\pixel{f}
	%
	 & = \pixel{\convolution{\Phi}{W^{\Phi}}} + \waveletSum \pixel{\convolution{\Psi^{j}}{W^{\Psi^{j}}}} \nonumber                 \\
	%
	 & = \braket*{\translation{\omega}\Phi}{W^{\Phi}} + \waveletSum \braket*{\translation{\omega}\Psi^{j}}{W^{\Psi^{j}}} \nonumber \\
	%
...
The properties of Slepian wavelets are reviewed here, which are often akin to the harmonic setting but not always.

\subsubsection{Localisation}\label{sec:chapter3_localisation}

Typically, harmonic wavelets are constructed through a tiling of the harmonic line.
Therefore, the value of \(j\) in the wavelets \(\Psi^{j}\) captures increasingly higher scale.
The Slepian setting, however, is less straightforward.
Here \(p\) is a measure of spatial concentration, the lower the value of \(p\) the more well-localised the Slepian function is.
...
Scale-dicretised wavelets on the sphere satisfy the following \emph{Parseval frame} property
%
\begin{align}
	A\norm{f}^{2} \leq
	%
...
%
\begin{align}\label{eq:chapter3_slepian_isotropic_noise}
	\variance{\pixel{W^{\varphi}}}
	%
	 & = \expval*{\abs{\pixel{W^{\varphi}}}^{2}} \nonumber                                                                                                                       \\
	%
...
\begin{align}
	...
	%
	 & = \sigma^{2} \harmonicSum \conj{\harmonic{(\slepian{S})}} \harmonic{(\slepian[']{S})} \nonumber                                                       \\
	%
	 & = \sigma^{2} \delta_{pp'},
\end{align}
%
...
In contrast to standard spherical wavelets --- where the scaling function is \emph{not} used in denoising --- with Slepian wavelets one treats both the scaling function and the wavelets similarly.
The Slepian wavelet scaling function can be relatively well-localised and is hence not a low-frequency representation of the signal (see \cref{sec:chapter3_localisation}).
...
A common approach to analysing data of this form is by using wavelets, which allow one to probe spatially localised, scale-independent features of the signal.
However, wavelet methods on the whole sphere introduce problems with data of this form as the wavelet coefficients are contaminated near the boundaries of the region.
Slepian wavelets offer a solution to this problem.

Slepian wavelets are based on the eigenfunctions of the Slepian spatial-spectral concentration problem on the sphere which are the basis functions of the region.
Through a tiling of the Slepian line one may define scale-discretised wavelets which lead to a theoretically exact wavelet transform in both the continuous and discrete settings.
Initially the \emph{sifting convolution}~\cite{Roddy2021} was defined on the sphere.
This work generalises the convolution beyond the spherical setting and can be used in \emph{any} basis.
Thus, one may perform convolutions over the incomplete sphere by using the Slepian functions as a basis.
...
Slepian wavelets can be used in any standard wavelet application such as convex optimisation and sparse regularisation.

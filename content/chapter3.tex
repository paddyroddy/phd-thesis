...
This translation is in contrast to the standard rotation as it considers two angles rather than three \hl{(\cf{} \mbox{\cref{fig:chapter2_rotation_sphere}})} and thereby its output remains on the sphere.
...
First, consider \hl{an} axisymmetric Gaussian \hl{in \mbox{\(\ell{}\)}} on the sphere, \ie{}
%
...
\hl{Here the effect of the translation on signals with odd symmetry is considered.
...
\hl{The effect of the translation on signals with even symmetry is now considered.
...
\hl{One seeks a signal with neither odd nor even symmetry.
...
\end{equation}
%
In effect, this function is \hl{an} axisymmetric Gaussian in \(\ell{}\) modulated by a Gaussian in \(m\).

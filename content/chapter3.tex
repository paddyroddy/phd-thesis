...
This chapter is adapted from~\autocite{Roddy2021}, in which I was the lead author.

\section{Introduction}

Many fields in science and engineering measure data on spherical manifolds, such as computer graphics~\autocite{Ramamoorthi2004}, planetary science~\autocite{Turcotte1981}, geophysics~\autocite{Simons2006}, quantum chemistry~\autocite{Choi1999}, cosmology~\autocite{Bennett1996}, and computer vision~\autocite{Cohen2018,Esteves2020,Cobb2021}.
...
Existing convolutions such as the \emph{isotropic convolution} (\eg{}~\autocite{McEwen2007,Wei2011,Kennedy2011}) and the \emph{left convolution}~\autocite{Kennedy2011,Driscoll1994} restrict themselves to an axisymmetric kernel (\ie{} kernels that are invariant under azimuthal rotation).
The \emph{directional convolution} has an output which is not on the sphere (\eg{}~\autocite{McEwen2007,Wandelt2001}).
Lastly, the \emph{commutative anisotropic convolution}~\autocite{Sadeghi2012,Khalid2012} and the \emph{directional convolution} are computationally demanding.
No existing spherical convolution satisfies all three desirable properties.

This chapter presents an alternative spherical convolution, the \emph{sifting convolution}, defined through the sifting property of the Dirac delta --- in analogy to the Euclidean definition.
...
\begin{equation}\label{eq:chapter3_spherical_harmonic}
	\pixel{\harmonic{Y}}
	%
	= \sqrt{\factor \frac{(\ell-m)!}{(\ell+m)!}} P^{m}_{\ell}(\cos(\theta)) \exp(im\phi),
\end{equation}
%
...
%
\begin{equation}\label{eq:chapter3_ylm_phase}
	\conj{\harmonic{Y}}
	%
	= {(-1)}^{m} Y_{\ell(-m)}
\end{equation}
...
such that
%
\begin{equation}\label{eq:chapter3_flm_phase}
	\conj{\harmonic{f}}
	%
	= {(-1)}^{m} f_{\ell(-m)}
\end{equation}
...
\end{align}
%
where the second line follows by \cref{eq:chapter3_ylm_phase}.
...
\end{align}
%
where \(\sqrt{4\pi}\pixel{Y_{00}}=1\) follows by \cref{eq:chapter3_spherical_harmonic}.
...
\end{align}
%
where the first line follows by the reality of the Dirac delta \cref{eq:chapter3_dirac_delta_real}, and the later lines follow by the phase conventions \cref{eq:chapter3_ylm_phase,eq:chapter3_flm_phase}.
...
In real space the isotropic convolution (\eg{}~\autocite{McEwen2007,Wei2011,Kennedy2011}) is
%
\begin{equation}
	\pixel{(f \odot g)}
	%
...
The definition of the left convolution~\autocite{Kennedy2011,Driscoll1994} in real space is
%
\begin{equation}
...
The definition of the commutative anisotropic convolution~\autocite{Sadeghi2012,Khalid2012} is
%
\begin{equation}
	\pixel{(f \oplus g)}
	%

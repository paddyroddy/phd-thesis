...
\begin{align}\label{eq:chapter3_dirac_delta_real}
	...
	%
	 & = \displaystyle\sum\limits_{\ell m'} \pixel[']{Y_{\ell m'}} \pixel{\conj{Y_{\ell m'}}} \nonumber{}                \\
	%
...
The \(\reducedWigner{\ell}{m'}{m}(\beta)\) are the \emph{reduced d matrices}, which are real in this phase convention and satisfy many symmetry relations.
One such symmetry relation is
%
...
\end{equation}
%
which is explicitly used later.
...
\begin{equation}
	(f \star g)(x)
	%
	= \displaystyle\int\limits_{\mathbb{R}^{2}} \dd{y} f(x-y) g(y),
\end{equation}
%
...
\begin{align}
	...
	%
	 & = \integrateRotation{\rho} f(\rho\eta) \integrateSphere{\omega} \pixel{g} \displaystyle\sum\limits_{m'=-\ell}^{\ell} \conj{\big( \wigner{\ell}{m'}{m}(\rho^{-1}) \pixel{Y_{\ell m'}} \big)} \nonumber{} \\
	%
	 & = \displaystyle\sum\limits_{m'=-\ell}^{\ell} g_{\ell m'} \integrateRotation{\rho} f(\rho\eta) \wigner{\ell}{m}{m'}(\rho),
\end{align}
%
...
\begin{align}
	...
	%
	 & = \axiHarmonic{g} \displaystyle\int\limits_{0}^{2\pi} \dd{\gamma} \integrateSphere{\omega} \pixel{f} \sqrt{\inverseFactor} \pixel{\conj{\harmonic{Y}}} \nonumber{} \\
	%
	 & = 2\pi \sqrt{\inverseFactor} \harmonic{f} \axiHarmonic{g},
\end{align}
%
...
\begin{equation}
	(f \circledast g) (\rho)
	%
	= \harmonicSum \harmonic{f} \displaystyle\sum\limits_{m'=-\ell}^{\ell} \conj{\big(\wigner{\ell}{m'}{m}(\rho) g_{\ell m'}\big)},
\end{equation}
%
as
%
...
\begin{equation}
	\pixel{(f \oplus g)}
	%
	= \harmonicSum \displaystyle\sum\limits_{m'=-\ell}^{\ell} \wigner{\ell}{m'}{m}(\phi,\theta,\pi-\phi) f_{\ell m'} \conj{\harmonic{g}},
\end{equation}
%
as
%
